...
library OrdersLib {
  ...
  function buyToPriceMemory(Order memory self, uint256 buy_amount) internal pure returns (uint256) {
  ...
  /// @param self The order itself
  /// @param buy_amount The amount of buy_token to calculate the price from
  function buyToPrice(Order storage self, uint256 buy_amount) internal view returns (uint256) {
    return buy_amount.mulDiv(SCALE_FACTOR, self.pay_amount);
  }
  /// @notice Converts the price and pay_amount of the order to the buy_amount
  /// @param self The order itself

  function priceToBuy(Order storage self) internal view returns (uint256) {
  ...
  function buyQuote(Order storage self, uint256 purchase) internal view returns (uint256) {
  ...
  function reversePrice(Order storage self) internal view returns (uint256) {
